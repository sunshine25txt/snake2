...
{
   // soft shadow
   SDL_Rect shadow = {rect.x + 6, rect.y + 8, rect.w, rect.h};
   SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
   SDL_SetRenderDrawColor(renderer, 0, 0, 0, 60);
   rounded_rect_draw(shadow, SDL_Color{0, 0, 0, 60}, 10);
   ...
   // main rounded rect
   rounded_rect_draw(rect, c, 10);

   // subtle inner border
   SDL_SetRenderDrawColor(renderer, max(0, c.r - 30), max(0, c.g - 30), max(0, c.b - 30), 200);
   SDL_Rect border = {rect.x + 1, rect.y + 1, rect.w - 2, rect.h - 2};
   SDL_RenderDrawRect(renderer, &border);
...
void question_popup_draw()
{
   ...
   // responsive panel size (similar to design image)
   int pw = min(820, width - 140);
   int ph = min(520, height - 160);
   SDL_Rect panel = {(width - pw) / 2, (height - ph) / 2, pw, ph};

   // panel shadow
   SDL_Rect shadow = {panel.x + 10, panel.y + 12, panel.w, panel.h};
   SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
   SDL_SetRenderDrawColor(renderer, 0, 0, 0, 45);
   rounded_rect_draw(shadow, SDL_Color{0, 0, 0, 45}, 18);

   // panel background
   SDL_SetRenderDrawColor(renderer, 250, 250, 250, 255);
   rounded_rect_draw(panel, SDL_Color{250, 250, 250, 255}, 18);

   // header: small "GAME" tag + big title centered-left
   string smallTitle = "GAME RULES";
   string title = game.isSnake ? "SNAKE CHALLENGE!" : "LADDER CHALLENGE!";

   // small tag centered at top
   int headerY = panel.y + 28;
   center_text_draw(smallTitle, panel.x + panel.w / 2, headerY, SDL_Color{120, 120, 140, 255}, medium_font ? medium_font : small_font);

   // big title (a little below small tag)
   center_text_draw(title, panel.x + panel.w / 2, headerY + 36, colors.bg2, gigantic_font ? gigantic_font : large_font);

   // timer chip: circular chip at top-right inside panel
   int chipR = 34;
   int chipCx = panel.x + panel.w - 60;
   int chipCy = panel.y + 48;
   SDL_Color chipBg = colors.p1;
   filled_circle_draw(chipCx, chipCy, chipR, chipBg);
   string tstr = to_string(game.popup_time_left) + "s";
   center_text_draw(tstr, chipCx, chipCy, colors.white, large_font ? large_font : medium_font);

   // question text area (left/top area)
   Question &q = questions[game.curr_question_index];
   int textLeft = panel.x + 48;
   int textRight = panel.x + panel.w - 48;
   int textTop = panel.y + 120;
   int wrapW = textRight - textLeft;

   // word-wrap the question using small_font
   vector<string> words;
   {
      string w;
   ...
   for (size_t i = 0; i < words.size(); ++i)
   {
      ...
      {
         text_draw(curLine, textLeft, lineY, colors.dark, small_font);
         lineY += lineH;
         curLine = words[i];
      }
      else
   ...
   // compute option rects and draw large pill-like option buttons
   int totalOpts = (int)game.popup_options.size();
   vector<SDL_Rect> optRects = compute_option_rects_for_popup(panel, totalOpts);

   // layout tweak: make options more pill-like (increase height a bit)
   for (int i = 0; i < (int)optRects.size(); ++i)
   {
      SDL_Rect r = optRects[i];
      // expand horizontally to make pills if only one row
      if (r.w < panel.w / 2 && totalOpts <= 3)
      {
         int extra = (panel.w - 2 * 48) - (r.w * totalOpts) - (10 * (totalOpts - 1));
         if (extra > 0)
         {
      ...
      SDL_Color bg = hover ? colors.bg1 : SDL_Color{245, 245, 245, 255};
      SDL_Color fg = colors.dark;
      // add left index label "A: " visually by prefix text
      string label = string(1, char('A' + i)) + ": " + game.popup_options[i];
      // pill drawing
      // make pill slightly taller
      r.h = max(r.h, 44);
      button_draw_prof(r, label, bg, fg, hover, medium_font);
   }

   // footer helper text
   center_text_draw("Click an option or press A/B/C... Timeout auto-submits", panel.x + panel.w / 2, panel.y + panel.h - 28, SDL_Color{120, 120, 120, 255}, small_font);
}
...
void settings_draw()
{
   ...
   center_text_draw("SETTINGS", width / 2, box.y + 40, colors.dark, gigantic_font ? gigantic_font : medium_font);
