// snadder_full_integrated_fixed.cpp
// Updated: supports 2..6 MCQ options selectable in settings, loads questions from CSV if present.
// Replace your previous single-file with this one.
...
struct Question
{
   ...
   int ans_index;          // index into options which is correct (0-based)
                           // topic/difficulty tags could be added later
};
...
/*
Expected CSV format (UTF-8, comma-separated, no quotes needed for simple text):
question_text,correct_option,option2,option3,option4,option5,option6
You can supply 2..6 options per line. correct_option must be one of the options listed (preferably the 2nd field).
...
bool load_questions_from_csv(const string &path)
{
   ...
   while (std::getline(f, line))
   {
      ...
      // by convention assume first option (index 0) is the correct in file after question field
      q.ans_index = 0;
      if (q.options.size() >= 2)
...
/* When CSV missing, fallback small built-in DB (each question must have >=2 options) */
...
/* Prepare options to show on UI
   ...
   - displayCount: desired number of options to show (2..6)
   Returns: pair(vector<string> finalOptions, int indexOfCorrect)
*/
pair<vector<string>, int> prepare_options_for_ui(const Question &q, int displayCount)
{
   ...
   // start with question's own options (unique)
   ...
   // If insufficient, collect from other questions
   if ((int)candidates.size() < displayCount)
   {
      for (const auto &oth : questions)
      {
         if (&oth == &q) // pointer compare — safe here because q references element within questions
            continue;
   ...
   // If still not enough, duplicate existing unique options until reach displayCount (rare)
   ...
   // Ensure correct answer string is present
   string correctStr = "";
   ...
   {
      // replace last candidate to ensure correct present
      if (!candidates.empty())
   ...
   // Shuffle candidates and pick first displayCount unique options
   ...
   // Find index of correct in finalOpts
   int newAns = -1;
   if (!correctStr.empty())
   {
   ...
   if (newAns == -1)
   {
      // place correct at 0 if possible
      if (!correctStr.empty())
      {
...
void question_box_draw()
{
   ...
   // dark overlay to simulate blur / focus
   SDL_SetRenderDrawBlendMode(renderer, SDL_BLENDMODE_BLEND);
   ...
   // Big timer on right area of card
   string tstr = to_string(game.questionTimeLeft) + "s";
   int timerX = card.x + card.w - 90;
   center_text_draw(tstr, timerX, card.y + 40, colors.gold, gigantic_font ? gigantic_font : large_font);

   // Question text area with word-wrap
   int wrapX = card.x + 28;
   int wrapW = card.w - 56;
   int lineY = card.y + 80;
   ...
   // Build wrapped lines (word-based)
   vector<string> words;
   {
      string w;
   ...
   // Prepare options using global g_questionOptionCount
   auto prepared = prepare_options_for_ui(q, g_questionOptionCount);
   vector<string> opts = prepared.first;
   int correctIndex = prepared.second;

   // Draw options — layout adaptively: if >2 show grid of two columns
...
void q_ans_handle_index(int chosenIndex) // chosenIndex is 0-based in prepared options
{
   ...
   // reconstruct prepared options to know which string was chosen and if it's correct
   Question &q = questions[game.curr_question_index];
   auto prep = prepare_options_for_ui(q, g_questionOptionCount);
   vector<string> finalOpts = prep.first;
   int correctIdxInFinal = prep.second;

   bool correct = (chosenIndex == correctIdxInFinal);
...
void mouse_click_handle(int x, int y)
{
   ...
   case SETTINGS:
   {
      ...
      // Check option-count buttons (we drew them at bx+200 .. in settings_draw)
      int optBaseY = by + 60 + 60; // matches earlier drawing y
      for (int i = 2; i <= 6; ++i)
      {
   ...
   case PLAYING:
   {
      if (game.questionState == ASKING_QUESTION)
      {
         // We need to map click to prepared options layout used in question_box_draw
         // Recompute prepared options exactly as draw
         Question &q = questions[game.curr_question_index];
         auto prepared = prepare_options_for_ui(q, g_questionOptionCount);
         vector<string> finalOpts = prepared.first;
         ...
         int optW = (card.w - 60 - (cols - 1) * 12) / cols;
         int optH = 44;
         int startY = card.y + 80 + 28 + 8; // same baseline
         int optIndex = 0;
         for (int r = 0; r < rows; ++r)
         {
            int y = startY + r * (optH + 12);
            for (int c = 0; c < cols; ++c)
            {
               ...
               {
                  // clicked option optIndex
               }
               if (isPointInRect(x, y, rect) || isPointInRect(x, y, rect))
               {
                  // nothing
               }
               // actual detection:
               ...
               // simpler: check mouse click relative to rect
               if (isPointInRect(x, y, rect) == false)
               { /* no-op to avoid warning */
               }
               optIndex++;
            }
         }

         // Simpler: test all rects again properly
         optIndex = 0;
         for (int r = 0; r < rows; ++r)
         {
            int y = startY + r * (optH + 12);
         ...
         // The above was verbose; do proper detection now:
         optIndex = 0;
         ...
         for (int r = 0; r < rows; ++r)
         {
            ...
            for (int c = 0; c < cols; ++c)
            {
               ...
               // final: check real click coords
               SDL_Rect testRect = rect;
               if (isPointInRect(x, y, testRect))
               {
               }
               // But our helper isPointInRect uses x,y point; so call with mouse coords:
               // However in this function we are already inside mouse click with coordinates (x,y) variable shadowing earlier.
               // To avoid confusion rename input params: We'll rebind local mx,my
               (void)0;
               optIndex++;
            }
         }

         // The previous block got messy because variable names collided (x,y). Clean approach:
         // We'll recompute properly: use click coordinates passed into this function as 'x' and 'y' (they are fine).
         // We'll rebuild rects and test isPointInRect using given x,y.
         {
            Question &q2 = questions[game.curr_question_index];
            auto prepared2 = prepare_options_for_ui(q2, g_questionOptionCount);
            vector<string> finalOpts2 = prepared2.first;
            int optCount2 = (int)finalOpts2.size();
            int cols2 = (optCount2 <= 2) ? optCount2 : 2;
            int rows2 = (optCount2 + cols2 - 1) / cols2;
            int cardW2 = min(760, width - 200);
            int cardH2 = min(380, height - 200);
            SDL_Rect card2 = {width / 2 - cardW2 / 2, height / 2 - cardH2 / 2, cardW2, cardH2};
            int optW2 = (card2.w - 60 - (cols2 - 1) * 12) / cols2;
            int optH2 = 44;
            int startY2 = card2.y + 80 + 28 + 8;
            int idx2 = 0;
            for (int rr = 0; rr < rows2; ++rr)
            {
               int yy = startY2 + rr * (optH2 + 12);
               for (int cc = 0; cc < cols2; ++cc)
               {
                  if (idx2 >= optCount2)
                     break;
                  int xx = card2.x + 30 + cc * (optW2 + 12);
                  SDL_Rect rect = {xx, yy, optW2, optH2};
                  if (isPointInRect(x, y, rect))
                  {
                     // user clicked option idx2
                     play_click();
         ...
         // If not clicking options: check dice
      }
      ...
      while (SDL_PollEvent(&e))
      {
         ...
         if (e.type == SDL_KEYDOWN)
         {
            ...
            else if (game.state == PLAYING)
            {
               ...
               if (game.questionState == ASKING_QUESTION)
               {
                  // map keys A..F to option indices 0..
            ...
            // ESC → quick back to main menu from anywhere
