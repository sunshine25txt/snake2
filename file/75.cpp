...
void question_box_draw()
{
   ...
   // লজিক: যদি স্কিপ না করে থাকে তবে "SKIP > New", আর করলে "GIVE UP"
   string btnText = game.hasSkipped ? "GIVE UP" : "SKIP > New";
   // কালার: স্কিপ বাকি থাকলে গোল্ডেন, না থাকলে লাল
   SDL_Color btnColor = game.hasSkipped ? colors.snake : colors.gold;
   if (game.state == PRACTICE_MODE)
   {
      btnText = "SKIP";
...
void start_question_challenge(int pos)
{
   ...
   // রিসেট: নতুন চ্যালেঞ্জে প্লেয়ার আবার স্কিপ করার সুযোগ পাবে
   game.hasSkipped = false;
...
void mouse_click_handle(int x, int y)
{
   ...
   case PLAYING:
   {
      ...
      if (game.questionState == ASKING_QUESTION)
      {
         ...
         if (isPointInRect(x, y, skipBtn))
         {
            ...
            {
               // ১. স্কিপ মার্ক করুন
               game.hasSkipped = true;

               // ২. নতুন প্রশ্ন লোড করুন
               game.curr_question_index = rand() % questions.size();
               ...
               // ৪. অপশন রিবিল্ড (Rebuild)
               game.question_ui_options.clear();
               auto p = prepare_options_for_ui(questions[game.curr_question_index], g_questionOptionCount);
               game.question_ui_options = p.first;
               game.question_ui_correct = p.second;

               // রিটার্ন করুন (যাতে ভুল হিসেবে গণ্য না হয়)
               return;
            }
            else
            {
               // দ্বিতীয়বার স্কিপ করলে -> ভুল উত্তর
   ...
   case PRACTICE_MODE:
   {
      ...
      // Compute wrapped text height to locate options (reuse same wrap logic as draw)
      ...
      // SKIP button (just advance to next question, treat as no-answer)
      SDL_Rect skipBtn = {card.x + card.w / 2 - 60, card.y + card.h - 50, 120, 35};
      if (isPointInRect(x, y, skipBtn))
      {
         if (game.soundEnabled && snd_click)
      ...
      // QUIT PRACTICE (maps to scoreboard view via -1 flag)
      SDL_Rect quitBtn = {width / 2 - 180, height - 70, 360, 55};
      if (isPointInRect(x, y, quitBtn))
      {
         if (game.soundEnabled && snd_click)
            Mix_PlayChannel(-1, snd_click, 0);
         // Use sentinel -1 to indicate finished/quit — drawing code already checks this.
         game.practice_question_index = -1;
         return;
      }

      // Also allow clicking a BACK button on empty topics (if you draw one)
      // (no-op otherwise)
      break;
   }
...
void practice_mode_draw()
{
   ...
   // Quit হলে Scoreboard দেখানো (final)
   if (game.practice_question_index == -1 && game.practice_total_answered > 0)
   {
      // ... draws overlay, shows Total, Correct, Wrong, Accuracy and message ...
      // (এই অংশটি তোমার ফাইলে আছে — scoreboard UI)
      return;
   }
...
int main(int argc, char **argv)
{
   ...
   else
   {
      ...
      // If we have a "full" dataset (>= 40), preserve the original block mapping (0..9 -> topic0, 10..19 -> topic1...)
      if (qcount >= (size_t)NUM_TOPICS * 10)
      {
         for (size_t i = 0; i < qcount; ++i)
         {
            if (i < 10)
               game.topic_questions[0].push_back((int)i);
            else if (i < 20)
               game.topic_questions[1].push_back((int)i);
            else if (i < 30)
               game.topic_questions[2].push_back((int)i);
            else
               game.topic_questions[3].push_back((int)i);
         }
      }
      else
      {
