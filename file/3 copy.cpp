...
struct Player
{
     ...
     bool skipUsed = false; // <-- added: allow one skip per player per game
};
...
void question_popup_draw()
{
     ...
     rounded_rect_draw(panel, colors.dark, 12);

     // title / question text
     int qidx = game.curr_question_index;
     string qtext = (qidx >= 0 && qidx < (int)questions.size()) ? questions[qidx].question : "Question";
     text_draw(qtext, px + 24, py + 18, colors.white, medium_font);

     // options area
     int opts = (int)game.popup_options.size();
     SDL_Rect optsArea{px + 20, py + 70, pw - 40, 180};
     vector<SDL_Rect> rects = compute_option_rects_for_popup(optsArea, opts);

     for (int i = 0; i < opts; ++i)
     {
          SDL_Rect r = rects[i];
          bool hover = (i >= 0 && i < (int)game.popup_hover.size()) ? game.popup_hover[i] : false;
          button_draw_prof(r, game.popup_options[i], colors.cell1, colors.white, hover, medium_font);
     }

     // timer / hint
     int elapsed = (int)((SDL_GetTicks() - game.popup_start_time) / 1000);
     int timeLeft = max(0, game.popup_time_limit - elapsed);
     string tm = "Time: " + to_string(timeLeft) + "s";
     text_draw(tm, px + pw - 110, py + 18, colors.white, small_font);

     // Skip button: show only if current player hasn't used skip yet
     bool canSkip = false;
     int p = game.current;
     if (p >= 0 && p <= 1)
          canSkip = !game.players[p].skipUsed;

     if (canSkip)
     {
          int bw = 120, bh = 40;
          question_skip_btn_rect = {px + pw - bw - 20, py + ph - bh - 18, bw, bh};
          button_draw_prof(question_skip_btn_rect, "Skip (once)", colors.gold, colors.dark, false, small_font);
     }
     else
     {
          question_skip_btn_rect = {0, 0, 0, 0}; // disable
          // draw disabled-looking button
          int bw = 120, bh = 40;
          SDL_Rect rb = {px + pw - bw - 20, py + ph - bh - 18, bw, bh};
          button_draw_prof(rb, "Skip (used)", colors.GRID, colors.dark, false, small_font);
     }
}
...
void about_draw()
{
     ...
     yy += 24;
     center_text_draw("• Engaging gameplay that reinforces C programming concepts.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("• Adjustable difficulty levels to challenge your skills.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("• Dynamic question and answer system.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("• Beautifully designed graphics and animations.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("• Upbeat sound effects and music.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("• Comprehensive rules and instructions.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("• About section with version info.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("• Easy restart after winning.",
                      width / 2, yy, colors.dark, small_font);
     yy += 40;

     center_text_draw("CONTROLS:", width / 2, yy,
                      colors.dark, medium_font);
     yy += 24;
     center_text_draw("• Mouse: Navigate menus, roll dice, select answers.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("• Keyboard:",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("  - A/B: Select answer in question challenge.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("  - SPACE: Roll dice.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("  - H: Toggle cell hints.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("  - ESC: Quit to main menu.",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("  - R: Restart game after winning.",
                      width / 2, yy, colors.dark, small_font);
     yy += 40;

     center_text_draw("DEVELOPER:", width / 2, yy,
                      colors.dark, medium_font);
     yy += 24;
     center_text_draw("Aditya Narayan",
                      width / 2, yy, colors.dark, small_font);
     yy += 18;
     center_text_draw("November 2025",
                      width / 2, yy, colors.dark, small_font);
     yy += 40;

     center_text_draw("THANK YOU FOR PLAYING!", width / 2, height - 50,
                      colors.gold, gigantic_font ? gigantic_font : medium_font);
}
...
void mouse_click_handle(int x, int y)
{
     ...
     case PLAYING:
     {
         // if a question popup is visible, handle popup interactions first
         if (game.popup_visible)
         {
             // check option buttons (existing logic likely already handles these) ...
             // check Skip button
             if (question_skip_btn_rect.w > 0 && isPointInRect(x, y, question_skip_btn_rect))
             {
                 request_skip_question();
                 return;
             }

             // existing popup click handling...
             // ...existing code...
         }

         // existing playing-state click handling...
         // ...existing code...
     }
