...
void question_box_draw()
{
   ...
   if (game.state == PRACTICE_MODE)
   {
      qidx = game.topic_questions[game.practice_topic][game.practice_question_index % game.topic_questions[game.practice_topic].size()];
...
void mouse_click_handle(int x, int y)
{
   ...
   case PLAYING:
   {
      ...
      if (game.questionState == ASKING_QUESTION)
      {
         ...
         int optW = (card.w - 60 - (cols - 1) * 12) / cols;
         int optH = 44;
         // Note: We need to match the lineY logic from drawing precisely,
         // but for simplicity, we assume lineY+18 is roughly consistent or hardcode startY.
         // A safer way is to assume the mouse click area is within the card.
         // Let's re-simulate the Y offset logic approx:
         // Title(34) + Timer(40) + Text(wrapped)..
         // To be robust, let's just loop checking valid clicks in the card area.
         // We know drawing starts options at:
         int lineY = card.y + 80;
         // Simulate wrapping to get lineY (FAST version)
         // ... skipping precise wrapping calc for hit detection can be risky.
         // BETTER: store the rects during draw? No, state machine is cleaner.
         // Let's copy the wrap calc:
         string textToWrap = questions[game.curr_question_index].question;
         // ... (Same wrapping logic as draw) ...
         int wrapW = card.w - 56;
         vector<string> words;
         string w;
         for (char ch : textToWrap)
   ...
   case PRACTICE_MODE:
   {
      ...
      string textToWrap = questions[game.topic_questions[game.practice_topic][game.practice_question_index % game.topic_questions[game.practice_topic].size()]].question;
      int wrapW = card.w - 56;
      int lineY = card.y + 80;
      vector<string> words;
      string w;
...
int main(int argc, char **argv)
{
   ...
   window = SDL_CreateWindow("Aditya - Snadder Game Ultimate", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
   if (!window)
   {
   ...
   // Ensure all sets have at least something
   if (game.topic_questions[0].empty())
      game.topic_questions[0].push_back(0);
