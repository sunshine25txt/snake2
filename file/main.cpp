...
void question_box_draw()
{
   ...
   button_draw(optionA, "A: " + q.optionA, colorA, colors.dark,
   ...
   button_draw(optionB, "B: " + q.optionB, colorB, colors.dark,
...
// Difficulty box used already; keep similar helpers if needed
...
void welcome_draw()
{
   ...
   center_text_draw("THE SNADDER GAME", width / 2,
                    welcome_box.y + 50, colors.bg2, gigantic_font);
   ...
   text_draw(subtitle, sx, welcome_box.y + 110, colors.dark, medium_font);
...
void UI_draw()
{
   ...
   for (int i = 0; i < 2; i++)
   {
      ...
      text_draw(score,
                ...
                game.players[i].color, medium_font);
   }
...
void settings_draw()
{
   ...
   button_draw(preBtn,
               "Pre-Game Countdown: " +
                   to_string(game.preGameDefaultCountdown) + " sec",
...
void about_draw()
{
   ...
   center_text_draw("THE SNADDER GAME", width / 2,
                    box.y + 50, colors.bg2, gigantic_font);
   ...
   SDL_Rect backBtn = {
       box.x + (box.w / 2) - 90, // (half of 180)
       box.y + box.h - padding_bottom - 40,
       180,
       35};
...
void difficulty_select_draw()
{
   ...
   center_text_draw("SELECT DIFFICULTY", width / 2,
                    box.y + 40, colors.bg2, gigantic_font);
...
void mouse_click_handle(int x, int y)
{
   ...
   case WELCOME:
   {
      SDL_Rect welcome_box = {width / 2 - 300, height / 2 - 200, 600, 400};

      SDL_Rect startBtn = {width / 2 - 100, welcome_box.y + 180, 200, 60};
      SDL_Rect rulesBtn = {width / 2 - 100, welcome_box.y + 250, 200, 50};
      SDL_Rect settingsBtn = {width / 2 - 100, welcome_box.y + 310, 200, 50};
      SDL_Rect aboutBtn = {width / 2 - 100, welcome_box.y + 370, 200, 40};
...
int main(int argc, char **argv)
{
   ...
   //  snd_timeout = Mix_LoadWAV("assets/sound/timeup.wav");
   ...
   // if (!snd_timeout)
   //  fprintf(stderr, "Warning: snd_timeout failed to load: %s\n", Mix_GetError());
   ...
   while (!quit)
   {
      ...
      if (game.state == PLAYING)
      {
         ...
         if (game.questionState == ASKING_QUESTION)
         {
            ...
            {
               // if (game.soundEnabled && snd_timeout)
               // Mix_PlayChannel(-1, snd_timeout, 0);
      ...
      case WINNER:
      {
         ...
         center_text_draw(win_text, width / 2,
                          height / 2 - 20, colors.gold, gigantic_font);
